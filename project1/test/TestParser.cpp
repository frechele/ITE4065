...
//---------------------------------------------------------------------------
static void assertPredicatEqual(PredicateInfo& pInfo, unsigned leftRel,
                                unsigned leftCol, unsigned rightRel,
                                unsigned rightCol)
{
...
//---------------------------------------------------------------------------
static void assertPredicateBindingEqual(PredicateInfo& pInfo, unsigned leftBind,
                                        unsigned leftCol, unsigned rightBind,
                                        unsigned rightCol)
{
...
//---------------------------------------------------------------------------
static void assertSelectBindingEqual(SelectInfo& sInfo, unsigned binding,
                                     unsigned col)
{
...
//---------------------------------------------------------------------------
static void assertFilterBindingEqual(FilterInfo& fInfo, unsigned binding,
                                     unsigned col, uint64_t constant,
...
//---------------------------------------------------------------------------
static void assertFilterEqual(FilterInfo& fInfo, unsigned rel, unsigned col,
                              uint64_t constant,
                              FilterInfo::Comparison comparison)
{
...
TEST(Parser, ParsePredicates)
{
    ...
    QueryInfo i;
    auto& predicates = i.predicates;
    auto& filters = i.filters;
    ...
    assertFilterBindingEqual(filters[0], 0, 1, 111,
                             FilterInfo::Comparison::Equal);
    assertFilterBindingEqual(filters[1], 1, 2, 222,
                             FilterInfo::Comparison::Less);
    assertFilterBindingEqual(filters[2], 1, 1, 333,
    ...
    assertFilterBindingEqual(filters[0], 0, 1, 111,
                             FilterInfo::Comparison::Equal);
    assertFilterBindingEqual(filters[1], 1, 2, 333,
                             FilterInfo::Comparison::Less);
}
//---------------------------------------------------------------------------
...
TEST(Parser, DumpSQL)
{
    ...
    auto sql = i.dumpSQL();
    ASSERT_EQ(
        "SELECT SUM(\"0\".c1), SUM(\"1\".c4) FROM r0 \"0\", r2 \"1\" WHERE "
        "\"0\".c1=\"1\".c1 and \"0\".c0=\"1\".c0 and \"0\".c1=5;",
        sql);
}
//---------------------------------------------------------------------------
